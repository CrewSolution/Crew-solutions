...
export function ApprenticeSignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const existingUsers = await getUsers(undefined, formData.email)
      ...
      const newUser: User = {
        id: `apprentice-${Date.now()}`, // ID will be generated by DB, but client needs a placeholder
        type: "apprentice",
        ...formData,
        createdAt: new Date().toISOString(), // Will be overwritten by DB
        profileComplete: true, // Will be overwritten by DB
        rating: 0,
        jobsCompleted: 0,
      }

      const createdUser = await saveUser(newUser) // This now calls the API

      setCurrentUser(createdUser) // Set the user returned from the API
